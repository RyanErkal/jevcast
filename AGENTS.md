...
Selected text, mail content, and dictation transcripts go to Luna only when the user turns on that kind in Settings › AI › Luna. Code checks each request against those switches, and every send is logged without its text.
